// src/lib/pdf.ts — JITO Green Legacy · Light Premium Theme · Large Readable Fonts
import { JITO_LOGO_B64, ENV_LOGO_B64 } from './logo-data';

// ─── Receipt PDF ────────────────────────────────────────────────────────────
export function generateReceiptPDF(data: {
  receiptNumber: string;
  donorName: string;
  donorEmail: string;
  donorPan?: string;
  amount: number;
  numberOfTrees: number;
  campaignName: string;
  paymentGatewayId?: string;
  date: Date;
}): string {
  const dateStr = data.date.toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' });

  return `<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width,initial-scale=1">
  <style>
    @import url('https://fonts.googleapis.com/css2?family=EB+Garamond:ital,wght@0,400;0,600;1,400&family=DM+Sans:wght@300;400;500;600&display=swap');
    *{margin:0;padding:0;box-sizing:border-box}
    body{font-family:'DM Sans',sans-serif;background:#fff;width:794px;margin:0 auto;color:#1a2e1a}
    .page{padding:48px}
    .border{border:3px solid #aecfa4;padding:36px;min-height:960px;position:relative;border-radius:4px}
    .inner-border{border:1px solid #d3e6cc;position:absolute;inset:10px;border-radius:2px;pointer-events:none}
    .header{display:flex;align-items:center;gap:14px;margin-bottom:24px;padding-bottom:18px;border-bottom:2px solid #eaf2e6}
    .logo{width:56px;height:56px;object-fit:contain;flex-shrink:0;background:transparent}
    .org-name{font-family:'EB Garamond',serif;font-size:22px;color:#264422;font-weight:600}
    .org-tag{font-size:13px;color:#448039;margin-top:3px}
    .receipt-title{text-align:center;font-family:'EB Garamond',serif;font-size:26px;color:#264422;font-style:italic;margin:16px 0 4px}
    .receipt-number{text-align:center;font-size:13px;color:#448039;margin-bottom:24px}
    .section{margin:16px 0}
    .section-title{font-size:11px;text-transform:uppercase;letter-spacing:2.5px;color:#448039;border-bottom:1px solid #eaf2e6;padding-bottom:5px;margin-bottom:12px;font-weight:600}
    .row{display:flex;margin:10px 0;font-size:14px}
    .label{width:190px;color:#448039}
    .value{flex:1;color:#264422;font-weight:600}
    .amount-box{background:#f6f9f4;border:2px solid #aecfa4;border-radius:10px;padding:20px;margin:20px 0;text-align:center}
    .amount-label{font-size:11px;color:#448039;text-transform:uppercase;letter-spacing:2px;font-weight:600}
    .amount-value{font-family:'EB Garamond',serif;font-size:40px;color:#264422;font-weight:700;margin:8px 0}
    .trees-badge{font-size:16px;color:#448039;font-weight:600}
    .tax-note{font-size:11px;color:#448039;text-align:center;margin:16px 0;font-style:italic;line-height:1.6}
    .footer{position:absolute;bottom:36px;left:36px;right:36px;text-align:center;font-size:11px;color:#448039;border-top:1px solid #eaf2e6;padding-top:12px}
    .watermark{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%) rotate(-30deg);font-family:'EB Garamond',serif;font-size:64px;color:rgba(68,128,57,.04);font-weight:900;text-transform:uppercase;pointer-events:none;white-space:nowrap}
  </style>
</head>
<body>
<div class="page">
  <div class="border">
    <div class="inner-border"></div>
    <div class="watermark">JITO GREEN LEGACY</div>
    <div class="header">
      <img src="${JITO_LOGO_B64}" class="logo" alt="JITO" style="background:transparent"/>
      <div>
        <div class="org-name">JITO Green Legacy</div>
        <div class="org-tag">A Family Tree Plantation Drive by Mumbai Zone and all JITO Chapters</div>
      </div>
    </div>
    <div class="receipt-title">Official Donation Receipt</div>
    <div class="receipt-number">Receipt No: <strong>#${data.receiptNumber}</strong> &nbsp;|&nbsp; Date: ${dateStr}</div>
    <div class="section">
      <div class="section-title">Donor Information</div>
      <div class="row"><span class="label">Name</span><span class="value">${data.donorName}</span></div>
      <div class="row"><span class="label">Email</span><span class="value">${data.donorEmail}</span></div>
      ${data.donorPan ? `<div class="row"><span class="label">PAN</span><span class="value">${data.donorPan}</span></div>` : ''}
    </div>
    <div class="section">
      <div class="section-title">Donation Details</div>
      <div class="row"><span class="label">Campaign</span><span class="value">${data.campaignName}</span></div>
      <div class="row"><span class="label">Trees Sponsored</span><span class="value">${data.numberOfTrees} Trees 🌳</span></div>
      <div class="row"><span class="label">CO₂ Removed/yr</span><span class="value">↓ ${(data.numberOfTrees * 22).toLocaleString('en-IN')} kg</span></div>
      ${data.paymentGatewayId ? `<div class="row"><span class="label">Transaction ID</span><span class="value">${data.paymentGatewayId}</span></div>` : ''}
      <div class="row"><span class="label">Payment Mode</span><span class="value">Online (Razorpay)</span></div>
    </div>
    <div class="amount-box">
      <div class="amount-label">Total Amount Donated</div>
      <div class="amount-value">₹${data.amount.toLocaleString('en-IN')}</div>
      <div class="trees-badge">🌱 ${data.numberOfTrees} Trees · JITO Green Legacy · Mumbai Zone</div>
    </div>
    <div class="tax-note">
      This receipt is issued as proof of donation and may be eligible for tax exemption under<br/>
      Section 80G of the Income Tax Act, 1961 · JITO Mumbai Zone · 80G Reg: [Registration Number]
    </div>
    <div class="footer">
      Authorised Signatory · JITO Green Legacy · Mumbai, Maharashtra, India<br/>
      This is a computer-generated receipt and does not require a physical signature.
    </div>
  </div>
</div>
</body>
</html>`;
}

// ─── Badge label from tree count ─────────────────────────────────────────────
function getBadgeLabel(trees: number): string {
  if (trees >= 108) return 'Forest Patron';
  if (trees >= 54)  return 'Grove Sponsor';
  if (trees >= 27)  return 'Family Grove';
  if (trees >= 11)  return 'Family Pack';
  return 'Individual Sponsor';
}

// ─── Certificate PDF — Light Premium Theme, Large Fonts ───────────────────────
export function generateCertificatePDF(data: {
  donorName: string;
  numberOfTrees: number;
  campaignName: string;
  dedicationName?: string;
  date: Date;
  receiptNumber: string;
  chapter?: string;
}): string {
  const dateStr = data.date.toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' });
  const co2     = data.numberOfTrees * 22;
  const badge   = getBadgeLabel(data.numberOfTrees);
  const chapter = data.chapter || 'Mumbai Zone';

  return `<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width,initial-scale=1">
  <style>
    @import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,500;0,700;1,500;1,700&family=Cinzel:wght@500;600;700&family=DM+Sans:wght@300;400;500;600&display=swap');

    * { margin:0; padding:0; box-sizing:border-box; }

    /* ── Landscape A4 page ── */
    html, body {
      width: 1123px;
      height: 794px;
      background: #fffef9;
      font-family: 'DM Sans', sans-serif;
      -webkit-print-color-adjust: exact;
      print-color-adjust: exact;
    }

    .page {
      width: 1123px;
      height: 794px;
      background: #fffef9;
      position: relative;
      overflow: hidden;
      display: flex;
      flex-direction: column;
    }

    /* ── Border system ── */
    .outer-border {
      position: absolute;
      inset: 14px;
      border: 2.5px solid #c9a84c;
      border-radius: 2px;
      pointer-events: none;
      z-index: 5;
    }
    .inner-border {
      position: absolute;
      inset: 21px;
      border: 1px solid rgba(201,168,76,0.35);
      border-radius: 1px;
      pointer-events: none;
      z-index: 5;
    }

    /* ── Corner ornaments ── */
    .corner {
      position: absolute;
      width: 52px;
      height: 52px;
      z-index: 6;
    }
    .c-tl { top: 30px;  left: 30px;  border-top: 2px solid #8a9e3a; border-left: 2px solid #8a9e3a; }
    .c-tr { top: 30px;  right: 30px; border-top: 2px solid #8a9e3a; border-right: 2px solid #8a9e3a; }
    .c-bl { bottom: 30px; left: 30px;  border-bottom: 2px solid #8a9e3a; border-left: 2px solid #8a9e3a; }
    .c-br { bottom: 30px; right: 30px; border-bottom: 2px solid #8a9e3a; border-right: 2px solid #8a9e3a; }

    /* ── Faint watermark ── */
    .watermark {
      position: absolute;
      top: 50%; left: 50%;
      transform: translate(-50%, -50%);
      font-family: 'Cinzel', serif;
      font-size: 88px;
      color: rgba(138,158,58,0.04);
      font-weight: 700;
      text-transform: uppercase;
      pointer-events: none;
      white-space: nowrap;
      letter-spacing: 10px;
      z-index: 1;
    }

    /* ── Main content ── */
    .content {
      position: relative;
      z-index: 10;
      width: 100%;
      height: 100%;
      display: flex;
      flex-direction: column;
      align-items: center;
      justify-content: space-between;
      padding: 110px 60px 28px;
    }

    /* ── TOP: Logo row ── */
    .logo-row {
      width: 100%;
      display: flex;
      align-items: center;
      justify-content: space-between;
    }

    .logo-wrap {
      display: flex;
      align-items: center;
      justify-content: center;
      width: 90px;
      height: 72px;
      background: transparent;
      flex-shrink: 0;
    }

    .logo-img {
      max-width: 90px;
      max-height: 72px;
      width: auto;
      height: auto;
      object-fit: contain;
      background: transparent;
      display: block;
      /* Remove any dark background artifacts */
      mix-blend-mode: multiply;
    }

    /* JITO logo has black background — use this to remove it */
    .logo-jito {
      /* white background logo — no blend mode needed */
    }

    .logo-center {
      text-align: center;
      flex: 1;
      padding: 0 16px;
    }

    .org-eyebrow {
      font-family: 'Cinzel', serif;
      font-size: 11px;
      letter-spacing: 4px;
      color: #8a9e3a;
      text-transform: uppercase;
      display: block;
      margin-bottom: 4px;
    }

    .org-tagline {
      font-size: 13px;
      color: #6a8050;
      letter-spacing: 0.5px;
      font-family: 'DM Sans', sans-serif;
    }

    /* ── Gold divider ── */
    .gold-divider {
      display: flex;
      align-items: center;
      gap: 10px;
      width: 100%;
      justify-content: center;
      margin: 4px 0;
    }
    .gold-line {
      flex: 1;
      max-width: 200px;
      height: 1px;
      background: linear-gradient(to right, transparent, #c9a84c, transparent);
    }
    .gold-diamond {
      width: 7px; height: 7px;
      background: #c9a84c;
      transform: rotate(45deg);
      flex-shrink: 0;
    }

    /* ── MIDDLE: Certificate body ── */
    .cert-body {
      text-align: center;
      width: 100%;
      flex: 1;
      display: flex;
      flex-direction: column;
      align-items: center;
      justify-content: center;
      gap: 6px;
    }

    /* Main title */
    .cert-title {
      font-family: 'Cinzel', serif;
      font-size: 30px;
      font-weight: 700;
      color: #1a2e0a;
      letter-spacing: 3px;
      line-height: 1.1;
    }

    .cert-sub {
      font-family: 'DM Sans', sans-serif;
      font-size: 13px;
      letter-spacing: 4px;
      text-transform: uppercase;
      color: #7a9e3a;
      margin-bottom: 4px;
    }

    /* "This certifies that" */
    .certifies-text {
      font-family: 'Playfair Display', serif;
      font-size: 18px;
      color: #5a7a3a;
      font-style: italic;
      margin: 2px 0;
    }

    /* Donor name — biggest element */
    .donor-name {
      font-family: 'Playfair Display', serif;
      font-size: 52px;
      font-weight: 700;
      font-style: italic;
      color: #1a2e0a;
      line-height: 1.05;
      border-bottom: 2px solid #c9a84c;
      padding-bottom: 8px;
      min-width: 420px;
      display: inline-block;
      margin: 4px 0;
    }

    /* Chapter line */
    .chapter-line {
      font-family: 'DM Sans', sans-serif;
      font-size: 20px;
      font-weight: 600;
      color: #7a9e3a;
      letter-spacing: 1px;
      margin: 2px 0;
    }

    /* Body description */
    .body-text {
      font-family: 'Playfair Display', serif;
      font-size: 17px;
      color: #3a5a2a;
      line-height: 1.7;
      max-width: 780px;
      margin: 2px auto;
    }

    .text-highlight { color: #1a2e0a; font-weight: 700; }
    .text-dedication { color: #7a9e3a; font-style: italic; font-size: 19px; }

    /* Sponsorship badge */
    .badge-wrap { margin: 4px 0; }
    .badge {
      display: inline-block;
      border: 2px solid #c9a84c;
      border-radius: 50px;
      padding: 6px 28px;
      font-family: 'Cinzel', serif;
      font-size: 12px;
      letter-spacing: 3px;
      text-transform: uppercase;
      color: #8a7020;
      background: linear-gradient(135deg, #fffef0 0%, #fdf8e1 100%);
      box-shadow: 0 2px 10px rgba(201,168,76,0.2);
    }

    /* ── Impact metrics ── */
    .impact-row {
      display: flex;
      gap: 28px;
      justify-content: center;
      margin: 4px 0;
    }

    .impact-item {
      text-align: center;
      padding: 8px 20px;
      background: #f4f8f0;
      border: 1px solid #d0e0c0;
      border-radius: 10px;
      min-width: 130px;
    }

    .impact-number {
      font-family: 'Cinzel', serif;
      font-size: 22px;
      font-weight: 700;
      color: #264422;
      display: block;
      line-height: 1.1;
    }

    /* CO2 down arrow — prominent */
    .co2-arrow {
      color: #4a8a3a;
      font-size: 22px;
      font-weight: 900;
      margin-right: 2px;
    }

    .impact-label {
      font-family: 'DM Sans', sans-serif;
      font-size: 10px;
      color: #7a9e3a;
      text-transform: uppercase;
      letter-spacing: 1.5px;
      margin-top: 3px;
      display: block;
    }

    /* ── BOTTOM: Footer row ── */
    .cert-footer {
      width: 100%;
      display: flex;
      justify-content: space-between;
      align-items: flex-end;
      border-top: 1px solid #e0eccc;
      padding-top: 10px;
    }

    .footer-item { text-align: center; min-width: 130px; }

    .footer-label {
      font-family: 'DM Sans', sans-serif;
      font-size: 9px;
      color: #8a9e3a;
      text-transform: uppercase;
      letter-spacing: 2px;
      display: block;
      margin-bottom: 3px;
    }

    .footer-value {
      font-family: 'DM Sans', sans-serif;
      font-size: 11px;
      color: #264422;
      font-weight: 600;
    }

    .seal {
      width: 52px; height: 52px;
      border: 2px solid #c9a84c;
      border-radius: 50%;
      display: flex;
      align-items: center;
      justify-content: center;
      font-size: 22px;
      margin: 0 auto 4px;
      background: #fffef0;
      box-shadow: 0 2px 8px rgba(201,168,76,0.15);
    }

    /* ── Mobile responsive ── */
    @media screen and (max-width: 1200px) {
      html, body { width: 100%; height: auto; }
      .page {
        width: 100%;
        height: auto;
        min-height: 100vh;
        padding: 20px;
      }
      .content { padding: 20px 24px 20px; gap: 8px; }
      .donor-name { font-size: 36px; min-width: auto; width: 100%; }
      .cert-title  { font-size: 22px; }
      .body-text   { font-size: 14px; }
      .chapter-line { font-size: 16px; }
      .impact-row  { flex-wrap: wrap; gap: 10px; }
      .logo-wrap   { width: 64px; height: 52px; }
      .logo-img    { max-width: 64px; max-height: 52px; }
      .certifies-text { font-size: 15px; }
      .impact-number { font-size: 18px; }
    }

    @media print {
      html, body { width: 297mm; height: 210mm; }
      .page { width: 297mm; height: 210mm; }
    }
  </style>
</head>
<body>
<div class="page">
  <!-- Borders -->
  <div class="outer-border"></div>
  <div class="inner-border"></div>
  <!-- Corners -->
  <div class="corner c-tl"></div>
  <div class="corner c-tr"></div>
  <div class="corner c-bl"></div>
  <div class="corner c-br"></div>
  <!-- Watermark -->
  <div class="watermark">CERTIFIED</div>

  <div class="content">

    <!-- ① Logo Row — perfectly aligned -->
    <div class="logo-row">
      <div class="logo-wrap">
        <img src="${JITO_LOGO_B64}"
          class="logo-img logo-jito"
          alt="JITO Logo"
          style="background:transparent;mix-blend-mode:multiply"/>
      </div>

      <div class="logo-center">
        <span class="org-eyebrow">JITO Green Legacy</span>
        <div class="org-tagline">A Family Tree Plantation Drive by Mumbai Zone and all JITO Chapters</div>
      </div>

      <div class="logo-wrap">
        <img src="${ENV_LOGO_B64}"
          class="logo-img"
          alt="JITO Environment & Sustainability"
          style="background:transparent"/>
      </div>
    </div>

    <!-- ② Gold divider (top logos are absolutely positioned above) -->
    <div class="gold-divider">
      <div class="gold-line"></div>
      <div class="gold-diamond"></div>
      <div class="gold-line"></div>
    </div>

    <!-- ③ Certificate title -->
    <div class="cert-body">
      <div class="cert-title">Certificate of Tree Sponsorship</div>
      <div class="cert-sub">Presented by JITO Mumbai Zone and all Chapters</div>

      <div class="gold-divider" style="margin:2px 0">
        <div class="gold-line" style="max-width:120px"></div>
        <div class="gold-diamond"></div>
        <div class="gold-line" style="max-width:120px"></div>
      </div>

      <div class="certifies-text">This certificate is proudly presented to</div>

      <!-- ④ Donor name — largest element -->
      <div class="donor-name">${data.donorName}</div>

      <!-- ⑤ Chapter — dynamic -->
      <div class="chapter-line">JITO Chapter: ${chapter}</div>

      <!-- ⑥ Body text -->
      <div class="body-text">
        for generously sponsoring
        <span class="text-highlight">${data.numberOfTrees} ${data.numberOfTrees === 1 ? 'Tree' : 'Trees'}</span>
        under the <span class="text-highlight">${data.campaignName}</span>
        ${data.dedicationName ? `<br/>in loving honour of <span class="text-dedication">${data.dedicationName}</span>` : ''}
      </div>

      <!-- ⑦ Sponsorship badge -->
      <div class="badge-wrap">
        <span class="badge">✦ ${getBadgeLabel(data.numberOfTrees)} ✦</span>
      </div>

      <!-- ⑧ Impact metrics with big CO2 -->
      <div class="impact-row">
        <div class="impact-item">
          <span class="impact-number">${data.numberOfTrees}</span>
          <span class="impact-label">Trees Planted</span>
        </div>
        <div class="impact-item">
          <span class="impact-number">
            <span class="co2-arrow">↓</span>${co2.toLocaleString('en-IN')} kg
          </span>
          <span class="impact-label">CO₂ Removed / Year</span>
        </div>
        <div class="impact-item">
          <span class="impact-number">25+ yrs</span>
          <span class="impact-label">Lasting Impact</span>
        </div>
      </div>
    </div>

    <!-- ⑨ Footer row -->
    <div class="cert-footer">
      <div class="footer-item">
        <div class="seal">🌳</div>
        <span class="footer-label">Verified Plantation</span>
        <span class="footer-value">Geo-tagged &amp; Monitored</span>
      </div>
      <div class="footer-item" style="text-align:center">
        <span class="footer-label">Date of Issue</span>
        <span class="footer-value" style="display:block;margin-bottom:6px">${dateStr}</span>
        <span class="footer-label">Certificate No.</span>
        <span class="footer-value">#${data.receiptNumber}</span>
      </div>
      <div class="footer-item">
        <div class="seal">✦</div>
        <span class="footer-label">Authorised By</span>
        <span class="footer-value">JITO Mumbai Zone</span>
      </div>
    </div>

  </div><!-- /content -->
</div><!-- /page -->
</body>
</html>`;
}


